...
public class DataSetUtils extends DataSetUtilsHelper {
    ...
    /**
     * convenience short-hand notation for getting error variable at a specific data
     * point index. Returns 0.0 if no errors are defined.
     *
     ...
     * @param dataSet the source data set
     * @param eType the error type
     * @param x the data set x-value for which the error should be
     *            interpolated
    ...
    /**
     * convenience short-hand notation for getting error variables. Returns 0.0 if
     ...
     * @param x the data set x-value for which the error should be
     *            interpolated
     * @param interpolate determines if the value at index or the interpolated value
     *            at x should be returned
    ...
    /**
     * convenience short-hand notation for getting error variables (if defined for
     * dataset)
     *
     * @param dataSet the source data set
     * @param eType the error type
    ...
    public enum Compression {
        ...
        GZIP,
        /**
         * ZIP compression. Allways reads from first entry in archive and writes to a
         * new entry with same filename as achive minus zip extension.
         */
        ZIP,
        /**
         * Plaintext csv data
    ...
    /**
     * Open a OutputStream that is backed by the appropriate compression and file
     * handling streams.
     *
    ...
    /**
     * Open a InputStream that is backed by the appropriate stream classes for the
     ...
     * {datafield;type;format} style placeholders. Datafield references a field in
     * the metadata as specified by the metaDataDataSet interface. The special field
     * "systemTime" can be used to use the current system time. Also the standard
     * DataSet fields can be used ("dataSetName", "xMin", "xMax", "yMin", "yMax").
     * The optional type field supports "string", "date", "int/long" and
     * "float/double", where "string" is the default. The optional format field can
     * be used to provide format strings. The default for date is SimpleDateFormat
     * "yyyyMMdd_HHmmss", for int and float it is printf's "%d" and "%e".
     *
     * @param dataSet A dataSet containing all the data field referenced in the
     *            filename pattern
     * @param fileName Filename (with "{metadatafield;type;format}" placeholders for
     *            variables)
     ...
     * @return Compression Enum for the extension of the supplied filename. Defaults
     *         to Compression.NONE
     */
    ...
    /**
     * Export the contents of the supplied dataSet to file as comma separated values
     * with an additional comment header containing metaData if existent.<br>
     * The filename can contain placeholders of the form
     * {metadatafield;type;format}, where metadatafield references a field in the
     * metadata as specified by the metaDataDataSet interface. The special field
     * systemTime can be used to use the current system time. Also the standard
     * DataSet fields can be used ("dataSetName", "xMin", "xMax", "yMin", "yMax").
     * The optional type field supports "string", "date", "int/long" and
     * "float/double", where "string" is the default. The optional format field can
     * be used to provide format strings. The default for date is SimpleDateFormat
     * "yyyyMMdd_HHmmss", for int and float it is printf's "%d" and "%e".<br>
     ...
     * @param fileName Filename (with "{metadatafield;type;format}" placeholders
     *            for variables)
     * @param compression Compression of the file (GZIP, ZIP or NONE). Supply AUTO
     *            or omit this value to use file extension.
     * @param binary true: whether to store data as binary or string
     * @return actual name of the file that was written or none in case of errors
     */
    public static String writeDataSetToFile(final DataSet dataSet, final Path path, final String fileName,
     ...
     *            transfer) instead of 64-bit doubles (DataSet default,
     *            higher precision)
     */
    private static void writeNumericBinaryDataToStream(final OutputStream outputStream, final DataSet dataSet,
        ...
        if (is3D) {
            ...
            buffer.append("$x;").append(dataType).append(';').append(gridDataSet.getDataCount(DIM_X));
            buffer.append("\n$y;").append(dataType).append(';').append(gridDataSet.getDataCount(DIM_Y));
            buffer.append("\n$z;").append(dataType).append(';').append(gridDataSet.getDataCount()).append('\n');
        } else {
        ...
        try {
            ...
            if (asFloat && !is3D) {
                ...
                int nX = gridDataSet.getDataCount(DIM_X);
                int nY = gridDataSet.getDataCount(DIM_Y);
                int nZ = gridDataSet.getDataCount();

                final ByteBuffer byteBuffer = getCachedDoubleArray(CACHED_WRITE_BYTE_BUFFER,
                        Float.BYTES * (nX + nY + nZ));
                for (int ix = 0; ix < nX; ix++) {
                    byteBuffer.putFloat((float) gridDataSet.getGrid(DIM_X, ix));
                }
                for (int iy = 0; iy < nY; iy++) {
                    byteBuffer.putFloat((float) gridDataSet.getGrid(DIM_Y, iy));
                }
                for (int iy = 0; iy < nY; iy++) {
                    for (int ix = 0; ix < nX; ix++) {
                        byteBuffer.putFloat((float) gridDataSet.getValue(gridDataSet.getNGrid(), ix, iy));
                    }
                }
                outputStream.write(byteBuffer.array());
                release(CACHED_WRITE_BYTE_BUFFER, byteBuffer);
                ...
                doubleBuffer.put(gridDataSet.getGrid(DIM_X));
                doubleBuffer.put(gridDataSet.getGrid(DIM_Y));
                doubleBuffer.put(gridDataSet.getValues(gridDataSet.getNGrid()));
                outputStream.write(byteBuffer.array());
                release(CACHED_WRITE_BYTE_BUFFER, byteBuffer);
            }
        } catch (final IOException e) {
    ...
    protected static void writeHeaderDataToStream(final OutputStream outputStream, final DataSet dataSet) {
        try {
            ...
            if (dataSet instanceof GridDataSet) {
                buffer.append("## statistics disabled for GridDataSet, not yet implemented\n");
            } else {
                try {
            ...
            if (is3D) {
                ...
                for (int iY = 0; iY < nY; iY++) {
                    for (int iX = 0; iX < nX; iX++) {
                        ...
                        buffer.append(',');
                        buffer.append(gridDataSet.getGrid(DIM_X, iX)); // x-coordinate
                        buffer.append(',');
                        buffer.append(gridDataSet.getGrid(DIM_Y, iY)); // y-coordinate
                        buffer.append(',');
                        buffer.append(gridDataSet.getValue(gridDataSet.getNGrid(), iX, iY)); // negative error in y
                        buffer.append('\n');
                    }
                }
    ...
    private static DataSet readNumericDataFromBinaryFile(final BufferedReader inputReader,
        ...
        if (inputFile.reachedSplit()) {
            ...
            if (valindex[4] > -1) { // 3D Dataset
                ...
                result = new DoubleGridDataSet(dataSetName, 2, new double[][] { x, y, z });
            } else { // 2D Dataset
                final double[] x = readDoubleArrayFromBuffer(toRead.get(valindex[0]).data32,
                        toRead.get(valindex[0]).data64);
                final double[] y = readDoubleArrayFromBuffer(toRead.get(valindex[1]).data32,
    ...
    protected static DataSet readNumericDataFromFile(final BufferedReader inputFile, final String dataSetName,
        ...
        try {
            if (is3D) {
                ...
                z.position(0);
                z.get(zArray);
                result = new DoubleGridDataSet(dataSetName, 2, new double[][] { xArray, yArray, zArray });
            } else {
